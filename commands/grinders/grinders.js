  ...
  async execute(message, args, client) {
    ...
    if (!args[0]) {
      const helpBed = new MessageEmbed()
        ...
        .addField("fh grinders add <id>/<@mention>", "Adds a new grinder to the grinders team.")
        .addField("fh grinders remove <id><@mention>", "Removes the user from the grinders team.")
        .addField("fh grinders days <days>", "Adds __x__ amount of days to the grinder's profile.")
        .addField("fh grinders pending", "Lists all the users who are yet to pay the grinder cash.")
        .setTimestamp()
    ...
    if (firstArg === 'add') {
      ...
      return message.channel.send(`Done! ${user} was added to the grinder team.`)
      ...
      return message.channel.send(`Done! ${user} was removed from the grinders team.`)

    } else if (firstArg === 'days' || firstArg === 'day') {

      args.shift()
      if (!args[0]) {
        return message.channel.send("Please specify the number of days.\n\nExample: \`fh grinders days 5\`")
      }
      if (isNaN(args[0])) {
        return message.channel.send(`Specify a number in days. Example: \`fh grinder days 1\``)
      }
      await message.reply("Mention the user in chat now.\n\nYou have 15 seconds.")
