...
module.exports = {
  ...
  async execute(message, args){
    ...
    if(!time) return message.channel.send("You must provide time.")
     if(isNaN(time[0][0]) && !time.endsWith('s') && !time.endsWith('m') && !time.endsWith('h') && !time.endsWith('d')) return message.channel.send(`You must provide a valid time.\nExample: \`fh blacklist ${target.id} 1h\``)

    const user = await blacklists.findOne({ userID: target })
    console.log(user)
    if(user) return message.channel.send("That user is already blacklisted.")  
