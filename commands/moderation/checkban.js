module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!message.member.roles.cache.has("848576301182877727") && !message.member.roles.cache.has("848580138970251314")) return;
        ...
        message.channel.send({
            embed: {
                title: 'Ban Info',
                color: 'RED',
                description: `User: \`${ban.user ? ban.user.tag : "Unknown"}\`(\`${ban.user.id}\`)`,
                fields: [
                    {
                        name: 'Reason',
                        value: ban.reason
