...
async function runAttack(message, targetTeam, client) {
  ...
  if (!attackerTeam) return message.reply('You are not in a team.');

  ensureSummerFight(targetTeam);

  if (attackerTeam._id.equals(targetTeam._id)) {
    return message.reply('You cannot attack your own team.');
  }

  await resolveExpiredAttack(targetTeam, message.channel);

  if ((targetTeam.lives ?? 5) <= 0) {
    return message.reply(`**${targetTeam.name}** has no lives left.`);
  }
  ...
  if (attackerTeam.summerFight.attacksUsed >= ATTACKS_PER_WINDOW) {
    ...
    return message.reply(
      `Your team has used all ${ATTACKS_PER_WINDOW} attacks. Attacks reset <t:${Math.floor(resetAt / 1000)}:R>.`
    );
  }

  if (hasPendingAttack(targetTeam)) {
    return message.reply(`**${targetTeam.name}** is already under attack.`);
  }
  ...
  if (isShieldActive(targetTeam)) {
    ...
    return message.channel.send(
      `**${attackerTeam.name}** attacked **${targetTeam.name}**, but it was auto-blocked by an active shield! ` +
        `Shield ends <t:${shieldEnds}:R>. **${targetTeam.name}** gained ${SCORE.SHIELD_BLOCK} points and **${attackerTeam.name}** lost ${Math.abs(SCORE.FAILED_ATTACK)} points.`
    );
  }
  ...
  return message.channel.send(
    `**${attackerTeam.name}** launched a surprise attack on **${targetTeam.name}**! ` +
      `**${targetTeam.name}** has 1 minute to use \`fh block\` or the attack succeeds.`
  );
}
...
async function sendTeamSelect(message, client) {
  ...
  if (!teams.length) return message.reply('There are no teams available to attack.');
  ...
  collector.on('end', (collected, reason) => {
    if (reason === 'idle') {
      selectMessage.edit({
        content: 'Attack cancelled. No team was selected.',
        components: []
      }).catch(() => {});
    }
  });
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (!targetTeam) return message.reply('That team does not exist.');
