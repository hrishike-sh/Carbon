...
module.exports = {
  ...
  async execute(button, client) {
    ...
    if (button.customId === 'giveaway-join') {
        ...
        button.message.edit({
          ...
          components: [
            new ActionRowBuilder().addComponents([
              new ButtonBuilder()
                .setLabel(`🎉 ${gaw.entries.length.toLocaleString()}`)
      ...
      let blRoles = [];
      blRoles =
        (await server.findOne({ guildID: button.guildId }))?.giveaway_config
          ?.blacklisted_roles || [];
      if (blRoles.length && button.member.roles.cache.hasAny(...blRoles)) {
        return button.reply({
          embeds: [
            new EmbedBuilder()
      ...
      if (gaw.requirements.length > 0) {
        ...
        if (!canJoin) {
          return button.reply({
            embeds: [
              new EmbedBuilder()
                .setTitle(`You cannot join this giveaway :frowning2:`)
                .setDescription(
      ...
      const giveawayMessageId = button.message.components[0].components[0].url
        .split('/')
        .slice(-1)[0];
