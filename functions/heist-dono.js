...
const Heists = require('../database/models/heistm')
let mongoUrl
...
class HeistDono {
    ...
    static async setURL(dbUrl) {
        ...
        if (mongoUrl)
            throw new TypeError('A database url was already configured.')
        mongoUrl = dbUrl
    ...
    static async addHeistDono(userId, guildId, messages) {
        if (!userId) throw new TypeError('An user id was not provided.')
        if (!guildId) throw new TypeError('A guild id was not provided.')
        if (messages == 0 || !messages || isNaN(parseInt(messages)))
            throw new TypeError(
                'An amount of messages was not provided/was invalid.'
            )
        const user = await Heists.findOne({ userID: userId, guildID: guildId })

        if (!user) {
            const newUser = new Heists({
                userID: userId,
                guildID: guildId,
                amount: messages,
            })
        ...
        await user
            .save()
            .catch((e) => console.log(`Failed to append messages: ${e}`))

        return user.amount
    }

    static async removeHeistDono(userId, guildId, messages) {
        if (!userId) throw new TypeError('An user id was not provided.')
        if (!guildId) throw new TypeError('A guild id was not provided.')
        if (messages == 0 || !messages || isNaN(parseInt(messages)))
            throw new TypeError(
        ...
        user.save().catch((e) =>
            console.log(`Failed to subtract messages: ${e}`)
        )
    ...
    static async fetchLeaderboard(guildId, limit) {
        ...
        var users = await Heists.find({ guildID: guildId })
    ...
    static async fetch(userId, guildId, fetchPosition = false) {
        if (!userId) throw new TypeError('An user id was not provided.')
        if (!guildId) throw new TypeError('A guild id was not provided.')
        const user = await Heists.findOne({
            userID: userId,
            guildID: guildId,
        })
        if (!user) return false
        ...
        if (fetchPosition === true) {
            const leaderboard = await Heists.find({
                guildID: guildId,
            })
                .sort([['amount', 'descending']])
                .exec()
